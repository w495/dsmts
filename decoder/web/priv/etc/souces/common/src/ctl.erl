...
%% Include files
%%
-include("../include/common.hrl").  %% for APP & FMT macros
...
action(reload_code, Node, []) ->
    ...
    Res = call(Node, {appmgr, reload_code, []}),
    io:format("~n~p~n", [Res]),
    ok;

action(reload_cfg, Node, []) ->
    io:format("Reloading config on node ~p ... ", [Node]),
    Res = call(Node, {appmgr, reload_cfg, []}),
    io:format("~n~p~n", [Res]),
    ok;
...
action(log_level, Node, []) ->
    io:format("Get session logging level on node ~p ...~n",
              [Node]),
    Res = rpc_call(Node, appmgr, log_level, [<<>>], 5000),
...
action(log_level, Node, Args) ->
    ...
    rpc_call(Node, appmgr, log_level, [BinArgs], infinity);

action(debug_opts, Node, Args) ->
    io:format("Set session debug flag ~p on node ~p ...~n",
              [Args, Node]),
    BinArgs = lists:map(fun list_to_binary/1, Args),
    rpc_call(Node, appmgr, debug_opts, [BinArgs], infinity);
