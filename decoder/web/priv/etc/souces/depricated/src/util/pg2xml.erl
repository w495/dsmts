...
mountXml([_H|_T]=Res, JsonObjName) ->
    ...
    %string:join([?XML_TOP, ?FMT("<~s>", [JsonObjName]), Str, ?FMT("</~s>", [JsonObjName])], ?DELIM);
    string:join([?XML_TOP, lists:append(["<", utils:to_list(JsonObjName), ">"]), Str, lists:append(["</", utils:to_list(JsonObjName), ">"])], ?DELIM);

mountXml(Res, JsonObjName) ->
    %string:join([?XML_TOP, ?FMT("<~s>", [JsonObjName]), Res, ?FMT("</~s>", [JsonObjName])], ?DELIM).
...
mountXmlTopless([_H|_T]=Res, JsonObjName) ->
    Str = string:join(Res, ?DELIM),
    %string:join([?FMT("<~s>", [JsonObjName]), Str, ?FMT("</~s>", [JsonObjName])], ?DELIM);
    string:join([lists:append(["<", JsonObjName, ">"]), Str, lists:append(["</", JsonObjName, ">"])], ?DELIM);
    
mountXmlTopless(Res, JsonObjName) ->
    %string:join([?FMT("<~s>", [JsonObjName]), Res, ?FMT("</~s>", [JsonObjName])], ?DELIM).
...
nameFields([{Name, [ValI | VT ]}|T]) when is_list(ValI) ->
    [encodeTopless([ValI | VT ], utils:to_list(Name))| nameFields(T)];
    
nameFields([{Name, Val}|T]) ->
%    [?FMT("<~s>~s</~s>", [Name, encodeValue(Val), Name]) | nameFields(T)];
    [lists:append(["<", utils:to_list(Name), ">", encodeValue(Val), "</", utils:to_list(Name), ">"]) | nameFields(T)];
...
test() ->
    utils:test(fun() ->
        ?FMT("<~s>~s</~s>", ["JOPA", "JOPA","JOPA"])
    end, 100000),
    utils:test(fun() ->
        lists:append(["<", "JOPA" ">", "JOPA", "</", "JOPA", ">"])
    end, 100000).
